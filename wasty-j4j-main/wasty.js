...
wasty.on("message", async msg => {
      ...
      if (msg.author.id === client.user.id) {
        ...
        if (msg.author.bot) {
          ...
          if (meyrnn === 1) {
            ...
            await sleep(7000)
            await msg.channel.send("https://discord.gg/fgR4Bv2vUf") // sunucunun davet linki 
            await sleep(10000)
  ...
  wasty.on("guildCreate", wastys => {
    sleep(1000);
    if (
      wastys.id === "589413911304863746"  //j4j sunucu id
    ) {
  ...
  wasty.on("ready", () => {
        setInterval(() => {
         let wastys = wasty.channels.get("845325111639801876") //j4j yapacağınız kanal idsi
  ...
  client.login("ODQzOTUwNzM4MTk1NjExNjk5.9prGA9tPRF8P7I-M0HFGH_hUjN0")